...
function ProductList() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get('https://ali-be.vercel.app/product');
  ...
  const addProduct = async () => {
    if (newProductName && newProductPrice && newProductAction) {
      try {
        const response = await axios.post('https://ali-be.vercel.app/product', {
          name: newProductName,
  ...
  const deleteProduct = async (productId) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`https://ali-be.vercel.app/product/${productId}`);
  ...
  const updateProduct = async () => {
    try {
      ...
      await axios.put(`https://ali-be.vercel.app/product/${editProductId}`, {
        name: newProductName,
        price: newProductPrice,
        action: newProductAction,
      });
